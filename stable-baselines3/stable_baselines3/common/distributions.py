...
from model.CategoricalMasked import CategoricalMasked
...
class MultiCategoricalDistribution(Distribution):
    ...
    def proba_distribution(
        ...
        if mask is None:
            ...
            if len(mask.shape) == 3:
                self.distribution = [
                    CategoricalMasked(logits=split, mask=~mask[:, i, :])
                    for i, split in enumerate(th.split(action_logits, list(self.action_dims), dim=1))
            ]
            else:
                self.distribution = [
                CategoricalMasked(logits=split, mask=~mask[i, :])
